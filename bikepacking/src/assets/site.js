...
(function () {
  ...
  function initPlanner(root) {
    ...
    /**
     * Realistische Tagesdistanz.
     *
     * Ausgangspunkt ist die Strecke, die du an einem guten Tag OHNE Gepaeck
     * schaffst. Davon bleiben beim Bikepacking erfahrungsgemaess rund 60
     * Prozent uebrig – der Rest geht fuer Gewicht, Untergrund, Hoehenmeter,
     * Auf- und Abbau und die Organisation drauf.
     */
    function calculate() {
      ...
      var surfaceFactor = { asphalt: 1.0, gemischt: 0.85, offroad: 0.7 }[surface] || 0.85;
      var levelFactor = { erste: 0.85, einige: 1.0, viele: 1.1 }[level] || 1.0;

      // Gepaeck: bis 6 kg kaum spuerbar, danach linear bis maximal −20 Prozent.
      var loadFactor = load <= 6 ? 1 : Math.max(0.8, 1 - (load - 6) * 0.018);

      // Mehrtagesmuedigkeit
      var dayFactor = days <= 2 ? 1 : days <= 4 ? 0.95 : days <= 7 ? 0.9 : 0.85;

      var capacity = refKm * 0.6 * surfaceFactor * levelFactor * loadFactor * dayFactor;

      // Hoehenmeter kosten Strecke: 100 Hm entsprechen grob 9 km in der Ebene.
      var hmCost = (hm / 100) * 9;
      var km = Math.max(15, capacity - hmCost);

      var speed = { asphalt: 19, gemischt: 16, offroad: 12.5 }[surface] || 16;
      speed = speed * loadFactor * levelFactor;
      var ride = km / speed;
      var climbTime = hm / 500; // grob eine halbe Stunde je 250 Hm zusaetzlich
      ...
      out.innerHTML =
        ...
        '<p>Bei ' + Math.round(hm) + ' Höhenmetern am Tag auf ' +
          ...
          'Für ' + days + ' Tage ergibt das etwa ' + de(Math.round(km * days)) + ' Kilometer Gesamtstrecke.</p>' +

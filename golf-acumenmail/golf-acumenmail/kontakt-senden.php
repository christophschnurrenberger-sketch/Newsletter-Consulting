<?php
/**
 * AcumenMail Golf – Kontaktformular-Handler (serverseitig, ohne Drittanbieter)
 * -----------------------------------------------------------------------
...
$CONFIG = [
    ...
    'subject'      => 'Neue Anfrage zur Club-Analyse (Golf-Newsletter)',
...
function send_confirmation(string $toEmail, string $name, array $CONFIG): void {
    ...
    $navy = '#143D28';   // Clubgrün statt Marineblau
    $red  = '#B08A3E';   // gedecktes Gold als Akzent
    ...
    $html = '<!DOCTYPE html><html lang="de"><head><meta charset="utf-8">'
        ...
        . '<span style="display:none!important;opacity:0;color:#FAF9F5;font-size:1px;line-height:1px;max-height:0;max-width:0;overflow:hidden;">Ihre Anfrage ist bei uns eingegangen – wir melden uns innerhalb von 24 Stunden (werktags).</span>'
        . '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#FAF9F5;padding:24px 12px;">'
        . '<tr><td align="center">'
        . '<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background-color:#ffffff;border-radius:10px;overflow:hidden;border:1px solid #E1DED5;">'
        // Header
        ...
        . '</tr></table></td></tr>'
        // Body
        . '<tr><td style="padding:32px;font-family:Arial,Helvetica,sans-serif;color:#3F4A43;font-size:15px;line-height:1.6;">'
        ...
        . confirmation_step('1', 'Prüfung', 'Ich sehe mir Ihren Club und Ihre bisherige Mitgliederkommunikation in Ruhe an.', $navy, $red)
        . confirmation_step('2', 'Kennenlernen', 'In einem kurzen, unverbindlichen Gespräch klären wir Ziele, Saison und Aufwand.', $navy, $red)
        . confirmation_step('3', 'Klare Empfehlung', 'Sie erhalten eine konkrete Einschätzung, welcher Schritt für Ihren Club zuerst lohnt.', $navy, $red)
        . '</table>'
        ...
        . '</td></tr>'
        // Footer
        . '<tr><td style="background-color:#FAF9F5;padding:20px 32px;border-top:1px solid #E0E6ED;font-family:Arial,Helvetica,sans-serif;color:#8A95A5;font-size:12px;line-height:1.6;">'
...
function confirmation_step(string $num, string $title, string $text, string $navy, string $red): string {
    return '<tr><td style="padding:0 0 14px;">'
        ...
        . '<span style="color:#3F4A43;font-size:14px;line-height:1.5;">' . htmlspecialchars($text, ENT_QUOTES, 'UTF-8') . '</span>'
        . '</td></tr></table></td></tr>';
}
...
$email   = post('email');
$phone   = post('phone');
$company = post('company');   // Name des Clubs / der Golfanlage
$role    = post('rolle');     // Funktion im Club
$members = post('mitglieder');// Größenordnung der Mitgliederzahl
...
if (mb_strlen($company) > 150) {
    $errors[] = 'Der Clubname ist zu lang.';
}
if (mb_strlen($role) > 100) {
    $errors[] = 'Die Angabe zur Funktion ist zu lang.';
}
// Nur die Werte aus dem Auswahlfeld zulassen – alles andere wird verworfen.
$memberOptions = ['', 'bis 300', '300 bis 700', '700 bis 1200', 'mehr als 1200'];
if (!in_array($members, $memberOptions, true)) {
    $members = '';
}
...
$lines = [
    'Neue Anfrage über das Kontaktformular von AcumenMail Golf',
    '------------------------------------------------',
    'Name:        ' . $name,
    'E-Mail:      ' . $email,
    'Telefon:     ' . ($phone !== '' ? $phone : '—'),
